...
Vervolgens kunnen we de correlatie van het kookboek opnieuwe berekenen met de correlatie-\term{functie} uit het \term{Hmisc}-pakket:
...
Het tweede voorbeeld is mogelijk doordat de uitvoer een object is in de vorm van een \term{matrix}. Hiervan wordt vervolgens de rij geselecteerd met de naam \texttt{depth} en de kolom met de naam \texttt{price}. Hierdoor krij je dus de \emph{p}-waarde die behoord bij de correlatie tussen deze twee variabelen. De werking is hier dus gelijk als bij de selectie van rijen en kolommen bij een \term{data.frame}.
