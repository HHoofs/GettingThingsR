...
\begin{tikzpicture}[scale=2, font=\sffamily]
  ...
  % N
	\fill [gray!30, label=Vector ] (1.5,.5) rectangle (3.4,-1.4);
	\node[text= black!60,anchor=south, text height=1.5ex, text depth=.1ex] (nlab) at (2.45,.5) {n};
	\node[fblock,align=center] (N11)  at (2, 0) {$n_{1,1}$};
	\node[fblock,align=center] (N12)  at (2.5, 0) {$n_{1,2}$};
	\node[fblock,align=center] (N13)  at (3, 0) {$n_{1,3}$};
	\node[fblock,align=center] (N21)  at (2, -.5) {$n_{2,1}$};
	\node[fblock,align=center] (N22)  at (2.5, -.5) {$n_{2,2}$};
	\node[fblock,align=center] (N23)  at (3, -.5) {$n_{2,3}$};
	\node[fblock,align=center] (N31)  at (2, -1) {$n_{3,1}$};
	\node[fblock,align=center] (N32)  at (2.5, -1) {$n_{3,2}$};
	\node[fblock,align=center] (N33)  at (3, -1) {$n_{3,3}$};
	\node[text= black!80, anchor=south, text height=1.5ex, text depth=.1ex] at (N11.north) {depth};
	\node[text= black!80, anchor=south, text height=1.5ex, text depth=.1ex] at (N12.north) {table};
	\node[text= black!80, anchor=south, text height=1.5ex, text depth=.1ex] at (N13.north) {price};
	\node[text= black!80, rotate=90, anchor=south, text height=1.5ex, text depth=.1ex] at (N11.west) {depth};
	\node[text= black!80, rotate=90, anchor=south, text height=1.5ex, text depth=.1ex] at (N21.west) {table};
	\node[text= black!80, rotate=90, anchor=south, text height=1.5ex, text depth=.1ex] at (N31.west) {price};
  % P
	\fill [gray!60, label=Vector ] (3.5,.5) rectangle (5.4,-1.4);
	\node[ anchor=south, text height=1.5ex, text depth=.1ex] (plab) at (4.45,.5) {P};
	\node[block,align=center] (P11)  at (4, 0) {$p_{1,1}$};
	\node[block,align=center] (P12)  at (4.5, 0) {$p_{1,2}$};
	\node[block,align=center] (P13)  at (5, 0) {$p_{1,3}$};
	\node[block,align=center] (P21)  at (4, -.5) {$p_{2,1}$};
	\node[block,align=center] (P22)  at (4.5, -.5) {$p_{2,2}$};
	\node[block,align=center] (P23)  at (5, -.5) {$p_{2,3}$};
	\node[block,align=center] (P31)  at (4, -1) {$p_{3,1}$};
	\node[block,align=center] (P32)  at (4.5, -1) {$p_{3,2}$};
	\node[block,align=center] (P33)  at (5, -1) {$p_{3,3}$};
	\node[anchor=south, text height=1.5ex, text depth=.1ex] at (P11.north) {depth};
	\node[anchor=south, text height=1.5ex, text depth=.1ex] at (P12.north) {table};
	\node[anchor=south, text height=1.5ex, text depth=.1ex] at (P13.north) {price};
	\node[rotate=90, anchor=south, text height=1.5ex, text depth=.1ex] at (P11.west) {depth};
	\node[rotate=90, anchor=south, text height=1.5ex, text depth=.1ex] at (P21.west) {table};
	\node[rotate=90, anchor=south, text height=1.5ex, text depth=.1ex] at (P31.west) {price};
  % Paths
	\path [line] (Overal) --  node [anchor=center, fill=white,align=center,sloped] {Filter~=~\$P} (plab);
	\path [line, dashed] (Overal) -- (nlab);
	\path [line, dashed] (Overal) -- (rlab);
\end{tikzpicture}
\caption{Schematische weergave van de werking van een lijst en het filteren ervan. De filter is in dit geval, \texttt{\$P}. Hierdoor wordt dus het object in de lijst geselecteerd met de naam \emph{P}.} \label{fig:schemaLijstFilter}
\end{figframe}
\end{figure}
...
\section{Kookboek}
\subsection{T-toets}
Nu we zicht hebben op de elementaire stappen binnen \term{R}, kunnen we enkele basale toetsen gaan uitvoeren. Als eerste zullen we, uiteraard, kijken naar de \term{t-toets} voor \'e\'en steekproef. Zo kunnen we bijvoorbeeld kijken of de gemiddelde waarde van de diamanten gelijk is aan \$ 4000,-. Hiervoor gebruiken de \term{functie} \func{t.test} op de volgende manier:
<<label="DiamantenPrijsTtest", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
t.test(x=diamanten$price, mu=4000)
@

De gemiddelde waarde is dus niet gelijk aan \$ 4000,-. In dit geval is er gebruik gemaakt van een tweezijdige toets met een \textalpha van 5\%. Om andere toetsingscriteria te gebruiken kunnen de standaard waardes worden aangepast (\texttt{?t.test}). Zo kan het \term{element} \texttt{alternative} de waardes \texttt{''two.sided''} (standaard waarde), \texttt{''less''}, of \texttt{''greater''} krijgen, om de richten van de hypothese aan te geven. Om het significantieniveau aan te passen kan de standaard waarde van het \term{element} \texttt{conf.level} worden aangepast. Deze waarde is gelijk aan 1 - \textalpha en geeft dus het percentage van het betrouwbaarheidsinterval van de t-waarde aan. Als we dus een rechtszijdige \term{t-toets} willen uitvoeren met \textalpha = .01 dan kan dat als volgt:
<<label="DiamantenPrijsTtest01", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
t.test(x=diamanten$price, mu=4000, alternative="greater", 
       conf.level=1-.01)
@

Zoals je ziet is er voor gekozen om het percentage van het betrouwbaarheidsinterval niet direct te geven maar door \term{R} te laten uitrekenen. Ook al was dat in dit geval natuurlijk net zo eenvoudig om zelf te doen, het geeft aan dat de waarde van een element ook ''indirect'' kan worden gegeven. 

Na de \term{t-toets} voor \'e\'en steekproef volgt logischerwijs de \term{t-toets} voor twee steekproeven. De dataset bevat momenteel echter nog geen variabele die uit twee groepen bestaat. Daarom gaan we eerst een dichotome variabele aanmaken. Laten we bijvoorbeeld het aantal karaat waaruit de diamanten bestaan categoriseren in een hoge en lage groep. Eerst voegen we een nieuwe, lege, variabele toe aan het dataframe:
<<label="DiamantenNewVariable", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
diamanten$caratC <- NA
@

Nu is er nieuwe variabele toegevoegd aan het dataframe. Momenteel bestaat deze variabele echter uitsluitend uit missende waardes, \texttt{NA}\footnote{NA staat voor \textit{Not Available}, niet aanwezig}. Nu willen we alle diamanten die beneden gemiddeld scoren m.b.t. het aantal karaats aan de groep ''laag'' toewijzen, dit kan (onder andere) als volgt:
<<label="DiamantenNewVariableLaag", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
gem_karaat <- mean(x=diamanten$carat, na.rm=TRUE)
diamanten[diamanten$carat < gem_karaat,"caratC"] <- "laag"
@

Te snel? Dan lopen we de bewerkingen nog even stap voor stap door. Eerst berekenen we het gemiddelde karaat-niveau en wijzen we deze waarde toe aan het \term{object} \texttt{gem\_karaat}. Als we dit dus opvragen krijgen we het gemiddelde karaat-niveau (\Sexpr{gem_karaat}). Vervolgens selecteren we alle rijen van het dataframe waarvan het aantal karaats lager ligt dan dit gemiddelde (\texttt{diamanten\$carat < gem\_karaat}). Daarnaast selecteren we de variabele waaraan we nieuwe waardes willen toevoegen (\texttt{''caratC''}). Eigenlijk worden alleen de cellen geselecteerd van de kolom met als variabele naam ''caratC'' waarvan de rijen een waarde hebben onder het gemiddelde karaats-niveau. Tot slot wordt de waarde toegewezen die deze cellen moeten krijgen (\texttt{''laag''}). Nu rest ons alleen nog om de andere, hoge, groep toe te wijzen:
<<label="DiamantenNewVariableHoog", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
diamanten[diamanten$carat >= gem_karaat,"caratC"] <- "hoog"
@

Zoals je ziet wordt er in plaats van \keys{>}, \keys{>=} gebruikt. Hiervoor is gekozen omdat anders diamanten met een waarde die exact gelijk ligt aan het gemiddelde geen waarde krijgen toegewezen. 

Ok, terug naar waar we mee bezig waren, een \term{t-toets} voor twee steekproeven:
<<label="DiamantenTtest", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
t.test(formula=price~caratC, data=diamanten)
@

Zoals verwacht ligt de gemiddelde waarde van diamanten met een hoog karaats gehalte een stuk hoger. Het \term{formula} element is een bijzonder element binnen \term{R}, en we zullen dit dan ook nog vaker tegenkomen (voor wie niet kan wachten; \texttt{?formula}). In dit geval betekent het, dat \texttt{price} wordt opgesplitst voor de verschillende waardes van \texttt{caratC}. 

Zoals we echter van SPSS(-handboeken) leren, is het van belang om te kijken of de varianties van de twee groepen wel gelijk zijn. Als dit immers niet het geval is moet er een gecorigeerde \term{t-toets} worden uitgevoerd. Als we echter kijken naar het aantal vrijheidsgraden dan zien we dat dit geen rond getal is, oftewel er is al sprake van een gecorigeerde \term{t-toets}. Om te controleren of dit terecht is, is de bekende Levene's test nodig om de homogeniteit van varianties te toetsen. Deze zit, vreemd genoeg, echter niet standaard in \term{R} en daarom moeten we hier een additioneel pakket (\term{car}) voor installeren, in dit voorbeeld via de syntax:

<<label="InstallSyntaxCar", tidy=FALSE, highlight=TRUE, warning=FALSE,  eval=FALSE>>=
install.packages("car")
@

Daarna moeten we dit pakket natuurlijk ook nog activeren:
<<label="RequireSyntaxCar", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
require(car)
@

Nu kunnen we kijken of de varianties aangaande de karaats-niveaus van de lage en hoge groep gelijk zijn:
<<label="DLevenesTtest", tidy=FALSE, highlight=TRUE>>=
leveneTest(y=price~caratC, data=diamanten)
@

De \texttt[newred]{Warning message} kunnen we negeren, deze wordt aangemaakt omdat de groep variabele niet een \term{factor} is (t.b.v. deze bewerking is wel als zodanig behandeld). Het blijkt dat het terecht is dat er een gecorigeerde \term{t-toets} is uitgvoerd aangezien de Levene's test significant is, \emph{p} < .05. Mocht dit niet het geval zijn geweest, dan hadden we een ''normale'' \term{t-toets} kunnen uitvoeren:
<<label="DiamantenTtestCorrect", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
t.test(formula=price~caratC, data=diamanten, var.equal=TRUE)
@

\subsection{AN(C)OVA}
Maar wat nu als we niet twee maar meerdere groepen hebben, dan komen we natuurlijk uit op de \term{ANOVA}. \term{R} heeft, uiteraard een ingebouwde \term{ANOVA} \term{functie}. Deze zullen we nu in twee stappen uitwerken. De eerste stap bestaat uit het  defineren van het model\footnote{het werken met dergelijke modellen is ook mogelijk bij de \func{t.test}, aangezien deze per definitie slechts twee varialen bevat werd er in dat geval gekozen voor de versimpelde versie. Daar waar AN(C)OVA's meestal echter een stuk complexer van aard zijn wordt hier wel voor de meer stapsgewijze methode gekozen}:
<<label="AnovaModel", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
model_anova <- lm(formula=price ~ cut, data=diamanten)
@

Het belangrijkste gedeelte bij het defineren van het model is de formule. Hier kan je namelijk specificeren wat de onafhankelijke en afhankelijke variabelen zijn, maar ook of er sprake is van een interactie effect en dergelijke. Alle variabelen voor de \keys{~} zijn de afhankelijke variabelen (in dit geval de prijs van de diamanten) en alle variabelen achter de \keys{~} zijn de onafhankelijke variabelen (in dit geval de kwaliteit van de diamanten). Tot slot wordt onder \texttt{data} aangegeven uit welk dataframe de variabelen komen.

De volgende stap bestaat uit het defineren van de bewerking die we willen uitvoeren over dit model. Zoals we later zullen zien is immers ook mogelijk dit model te gebruiken voor een regressie-analyse. Om een anova uit te voeren gebruiken we de \term{functie} \func{aov}, als volgt:
<<label="AnovaAov", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
out_anova <- aov(formula=model_anova)
out_anova
@

Nu hebben we echter slechts de helft van het verhaal. Naast deze algemene duiding van het model in de vorm van de kwadratensommen en vrijheidsgraden, willen we namelijk ook de statistieken die bij de F-verdeling van een ANOVa horen. Om deze te vekrijgen gebruiken we de functie \func{summary}, die we ook al eerder zijn tegen gekomen bij de beschrijvende statstiek:
<<label="AnovaAovSum", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
summary(out_anova)
@

\begin{tip}[\'E\'en functie, oneindig veel bewerkingen]
Zoals we hebben kunnen zien in hoofdstuk XXX geeft de \term{functie} \func{summary} een beknopte beschrijving weer van de variabelen in een bepaald dataframe (\texttt{summary(diamanten)}). In dit huidige hoofdsstuk aangaande XXX, blijkt echter dat deze zelfde functie ook gebruikt kan worden om de resultaten te verkijgen van een ANOVA model (\texttt{summary(out\_anova)}). In al deze syntax hebben echter niet aangegeven wat de invoer dan wel de gewenste uitvoer er precies uitzag. 

Dit hoeven we echter bij deze functie niet te vertellen, omdat de \term{functie} \func{summary} een van de vele functies is die zelf ''herkend'' wat voor een soort invoer er wordt gegeven. Al naar gelang deze invoer verschilt te bewerking van de \term{functie} en dus ook de daarop volgende uitvoer. Het is echter niet zo dat alle functies als haarlemmer wonderolie werken. Sommige kunnen bijna alle  soorten objecten verwerken (bijv. \func{summary}), andere enkele soorten (\func{head}), terwijl de meeste functies slechts \'e\'en soort opbject kunnen verwerken.
\end{tip}

Conclusie; er is een significant effect van de kwaliteit van de diamanten op de prijs van de diamanten. Om echter te conclusie te vervolledigen is het belangrijk om een post-hoc test uit te voeren om te kijken welke groepen nu precies van elkaar verschillen:
<<label="AnovaHSD", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
TukeyHSD(out_anova)
@

Het blijkt dat bijna alle groepen van elkaar verschillen op enkele combinaties na. Bij dit alles is het echter van belang te vermelden dat \term{R} als standaard methode bij ANOVA Type I gebruikt. Dit in tegenstelling tot de Type II \& III die veelal bij andere statistiek programma's worden gebruikt. Deze Type's zijn ook te gebruiken in \term{R}, wat verder beschreven wordt bij het kijkje in de keuken. 

Omdat de mogelijkheden bij een ANOVA natuurlijk eindeloos zijn maar het aantal pagina's van dit boek niet, zal er ter illustratie nog paar modellen worden getoond. Een veel voorkomende toepassing is natuurlijk het gebruik van covariaten:
<<label="Ancova", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
model_ancova <- lm(formula=price ~ cut + carat, data=diamanten)
out_ancova <- aov(formula=model_ancova)
summary(out_ancova)
@

Hierbij wordt het aantal karaat van de diamant automatisch als covariaat gepakt, omdat het een continue maat is. Dit laat ook meteen zien dat als dit \textbf{niet} het geval is, dit ook duidelijk gespecificeerd moet worden. Voor de zekerheid is het daarom verstandig om factoren, zeker als ze numeriek zijn, altijd (nogmaals) als zodanig te specificeren in het model:
<<label="Ancova_F", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
model_ancova <- lm(formula=price ~ factor(cut) + carat, data=diamanten)
out_ancova <- aov(formula=model_ancova)
summary(model_ancova)
@

Tot slot zullen nog twee modellen worden geilustreerd waarin twee onafhankelijek variabelen worden opgenomen. Waarbij de eerste alleen de twee directe effecten toetst, terwijl de tweede ook het interactie effect toevoegd:
<<label="Anova2", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
# # Zonder interactie
model_anova2 <- 
  lm(formula=price ~ factor(cut) + factor(color),
     data=diamanten)

out_anova2 <- aov(formula=model_anova2)
summary(out_anova2)

# # Met interactie (* i.p.v. +)
model_anova2int <- 
  lm(formula=price ~ factor(cut) * factor(color),
     data=diamanten)
out_anova2int <- aov(formula=model_anova2int)
summary(out_anova2int)
@

\subsection{Regressie}
Het verschil tussen een ANOVA en een regressie analyse is miniem. Dit wordt mooi ge\"ilustreerd in \term{R} aangezien we precies dezelfde modellen kunnen gebruiken als we hebben gebruikt bij de ANOVA analyses. In plaats van de bewerking \func{aov} gebruiken we nu echter direct \func{summary}. Eerst moeten we echter er voor zorgen dat de factor niet langer als geordend wordt gezien, anders wordt deze alsnog linear gemodelleerd (zie pagina XXX voor verdere uitleg). Daarom maken we een nieuwe variabele aan om te gebruiken voor de lineaire regressie om deze vervolgend te gebruiken in een regressie-model:

<<label="Regressie", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
diamanten$cut_reg <- factor(diamanten$cut,ordered=FALSE)
model_reg <- lm(formula=price ~ cut_reg, data=diamanten)
summary(model_reg)
@

Hierbij wordt de variabele automatische gehercodeerd in dummy variabelen, waarbij het eerste level van de factor (''Fair'') wordt gebruikt als referentie categorie. Het blijkt dus dat alleen een ''Premium'' kwaliteit gerelateerd is aan duurdere diamanten t.o.v. van een ''Fair'' kwaliteit. Terwijl alle andere effecten juist de andere kant opgaan. Uiteraard is het niet echt zinvol om een regressie te draaien op enkel een ordinale variabele, dus laten we daarom ook nog wat continue variablen toe voegen:

<<label="Regressie2", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
model_reg2 <- lm(formula=price ~ cut_reg + carat + depth, data=diamanten)
summary(model_reg2)
@

Als we dus het aantal karaat alswel de ''diepte'' van de diamant meenemen als controle variabelen dan zien we dat de effecten meer de richting op gaan die we verwachten. Net als bij de ANOVA modellen worden ook hier continue variabelen automatisch als zodanig behandeld, als is het ook heir verstandig om dit altijd nog eens extra toe tevoegen aan de formule. Daarnaast is het ook bij regressie analyse mogelijk om interactie effecten toe te voegen, middels het gebruik van \keys{*}. 

\section{Kijkje in de keuken}
\subsection{AN(C)OVA}
 maar deze zullen we hier niet behandelen. De reden hiervoor is dat de standaard\footnote{Een \term{functie} die standaard in \term{R} zit noemen we ook wel een \term{base}-\term{functie}. \term{base} zou je eigenlijk kunnen zien als een \term{pakket} dat automatisch geladen wordt.} \term{functie} moeilijk te gebruiken is in combinatie met de gebruikelelijke type II of III kwadratensom (zoals gebruikelijk in bijvoorbeeld SPSS). Daarom gebruiken we de \term{ANOVA} \term{functie} uit het \term{car} \term{pakket}; \func{Anova}. Stel bijvoorbeeld dat we nogmaals naar de waarde van de diamanten willen kijken maar nu in relatie met de kwalitiet van de stenen:
<<label="DiamantenAnova", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
model <- lm(formula=price~cut, data=diamanten)
Anova(mod=model)
@

Voor de overzichtelijkheid is de bewerking hier in twee stappen uitgevoerd. Eerst wordt het model gespecificeerd, in dit geval dus de prijs als functie van de kwalieit van de steen en het bijbehorende dataframe. Daarna wijzen we dit model toe aan \func{Anova}, welke als standaard type II gebruikt. Als we dit willen aanpassen kan dat als volgt:
<<label="DiamantenAnovaType3", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
Anova(mod=model, type="3")
@

Ook hier is er een significant verschil tussen de groepen. Om te kijken welke groepen precies verschillen voeren we een post-hoc toets uit.

\chapter{Intermezzo: Op naar de sterren, en daarvoorbij}
Uiteraard is er nog veel meer mogelijk naast deze elementaire statistische toetsen. Zo is het uiteraard ook mogelijk een MANOVA te doen:
<<label="MANOVA", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
# # Afhankelijke variabelen samenvoegen
DV  <- cbind(x=diamanten$x,y=diamanten$y,z=diamanten$z)

# # MANOVA model
fit <- manova(IV ~ diamanten$cut + diamanten$color)

# # MANOVA output
summary(fit)

# # Aparte ANOVA voor iedere afhankelijke variabele
summary.aov(fit)
@

Of logistische regressie:
<<label="Log regressie", tidy=FALSE, highlight=TRUE, warning=FALSE>>=
# # Hercoderen naar numerieke 0 en 1
diamanten$caratC[diamanten$caratC=="laag"] <- 0
diamanten$caratC[diamanten$caratC=="hoog"] <- 1
diamanten$caratC <- as.numeric(diamanten$caratC)

# # Regressie uit de binominale (logistische) familie
model_log <-
  glm(caratC ~ price + table, data=diamanten, family=binomial())
...
Aangezien dit boek echter is bedoeld om een introductie te geven in \term{R} zijn dergelijke analyses al te specifiek aangezien ze waarschijnlijk niet voor iedereen van toepassing. De bedoeling van dit boek is, zoals in de introductie is vermeld, te leren fietsen zonder zijwieltjes. Hopelijk komt dit moment steeds dichterbij, want een oud-Hollandse wijsheid leert immers: ''Fietsen verleer je nooit.'' Nou is dit meestal juist niet het geval bij \term{R} aangezien het, zeker in het begin, toch iedere keer weer even inkomen is. 

Hopelijk doet dit boek in dergelijke momenten goed dienst om je snel weer op weg te helpen. Als je dan uiteindelijk weer op eigen benen staat (zonder zijwieltjes kan fietsen) is het internet en andere boeken de plek voor verder exploratie van de mogelijkheden en toepassing binnen \term{R}. 

De volgende hoofdstukken zullen daarom nog enkele belangrijke bewerkingen laten zien binnen \term{R};
\begin{itemize}
  \item Loops and if-statements, aan de hand van een \term{survival}-analyse,
  \item Het cre\"eren en inzien van functies,
  \item Data herstructureren, en
  \item Figuren ontwerpen.
\end{itemize}

Om vervolgens af te sluiten met een hoofdstuk waarin uiteen word gezet welke mogelijkheden erzijn binnen de \term{R}-gemeenschap om alledaagse en meer complexe problemen binnen enkele stappen opgelost te krijgen. Dit laatste hoofdstuk zal je dan ook, indirect, helpen om de meer complexe analyses uit te voeren.

\chapter{Survival}

\chapter{Functies nader bekeken}

\chapter{Data herstructureren}

\chapter{Figuren}

\chapter{Verder met \term{R}}
