...
\subsection{AN(C)OVA}
Maar wat nu als we niet twee maar meerdere groepen hebben, dan komen we natuurlijk uit op de \term{ANOVA}. \term{R} heeft een ingebouwde \term{ANOVA} \term{functie}, maar deze zullen we hier niet behandelen. De reden hiervoor is dat de standaard\footnote{Een \term{functie} die standaard in \term{R} zit noemen we ook wel een \term{base}-\term{functie}. \term{base} zou je eigenlijk kunnen zien als een \term{pakket} dat automatisch geladen wordt.} \term{functie} moeilijk te gebruiken is in combinatie met de gebruikelelijke type II of III kwadratensom (zoals gebruikelijk in bijvoorbeeld SPSS). Daarom gebruiken we de \term{ANOVA} \term{functie} uit het \term{car} \term{pakket}; \func{Anova}. Stel bijvoorbeeld dat we nogmaals naar de waarde van de diamanten willen kijken maar nu in relatie met de kwalitiet van de stenen:
