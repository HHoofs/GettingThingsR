...
In de loop van dit boek zullen nog vele varianten en alternatieve manieren van filteren de revu passeeren. Het doel is echter altijd gelijk, een deel van een object selecter en hier eventueel een bewerking op uitvoeren. 
% Define block styles http://www.texample.net/tikz/examples/simple-flow-chart/
    %Picture
\begin{figure}[!h]
\centering
\begin{tikzpicture}[scale=2]
  ...
  \draw[gray, fill=gray!20] (-.5,-.7) rectangle (4,-1.3);
	...
	\node[block,align=center] (fil1)  at (0, -1) {X};
    	\node[block,align=center] (fil2)  at (1, -1) {V};
    	\node[block,align=center] (fil3)  at (2, -1) {X};
    	\node[block,align=center] (fil4)  at (3.5, -1) {V};
