...
%Floats bovenaan als ze alleen zijn
\makeatletter
\setlength{\@fptop}{0pt}
\setlength{\@fpbot}{0pt plus 1fil}
\makeatother

%Eigen floats en frames
\usepackage{float}
...
\specialterms{R,RStudio,foreign,diamanten,factor,object,Inf}
...
\newcommand\func[1]{%
  \@ifundefined{specialfunc@\detokenize{#1}}
    {\texttt[black]{#1}}{\texttt[textblue]{#1}\index{#1@\textsl{#1}} \global\expandafter\let\csname specialfunc@\detokenize{#1}\endcsname\relax}%
}
\makeatother

\specialfunc{require,library,read.spss}
...
thm$highlight <- 
  gsub(pattern="textcolor[rgb]{0.69,0.494,0}","textcolor[rgb]{0.47,0.27,0}",thm$highlight,fixed=TRUE)
...
Echter, zoals vermeld staat bij de Tip aangaande het installeren van pakketten is installatie van een pakket slechts per PC slechts een keer nodig. Daarom is het voor de reproduceerbaarheid van de \texttt{syntax} geen must om het hierin op te nemen. Wat dus wel een must is voor de repoduceerbaarheid is het includeren van in de syntax van het inladen van het pakket. Naast de gebruikte functie \func{require} is het ook mogelijk de functie \func{library} te gebruiken--er zit geen enkel verschil tussen beiden. Tot slot nog een kleine waarschuwing bij het gebruik van meerdere pakketen tegelijk. Over het algemeen bevatten pakketten geen overeenkomstige fucntie benamingen. Soms komt dit echter wel voor, in een dergelijk geval wordt de functie gebruikt van het pakket dat als laatste wordt ingelezen.
...
De naam van het element kan worden weggelaten als de volgorde van invoer overeenkomt met de standaard volgorde van de functie. Aangezien \texttt{''diamanten.sav''} dus het eerste stukje invoer is wordt dus dit dus automatische gelinkt aan het eerste element (\texttt{file}) van de functie (onthoud \texttt{?read.spss}). Daarom is het ook niet mogelijk om de volgende, nog kortere, variant te gebruiken:
...
Hierop zijn natuurlijk talloze variaties mogelijk, die in het \textbf{Kijkje in de keuken} verder worden toegelicht. Wel is het benoemingswaardig dat de rijnummers worden getoond van het orginele dataframe, de eerste rij die dus voldoet aan het gekozen criteria is dus de 6\textsuperscript{e} rij. 
...
Voor \term{factor}- en \term{karakter}-variabelen behoren de volgende opties tot de mogelijkheden:
